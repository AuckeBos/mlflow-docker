...
class AbstractStorage(ABC):
    ...
    def _find(
        ...
        Parameters:
            ...
            query: Query, returned by convert_query.
    ...
    def find(
        ...
        """
        Find rows in a table that match the query. Convert the query and call _find
        ...
        return self._find(table, query, sort, asc)
    ...
    def upsert(self, data: List, table_name: str, key_col: str, timestamp_col: str) -> None:
        """
        Add updated_at, and then call _upsert.
        """
        updated_at = now().isoformat()
        data = map(lambda x: {**x, "updated_at": updated_at}, data)
        self._upsert(data, table_name, key_col, timestamp_col)
